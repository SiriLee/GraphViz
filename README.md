...
每行一条边或一个孤立点，支持 `#` 行注释和空行。顶点名由字母、数字、中文、下划线组成；含空格、`-`、`<`、`>`、`"`、`#` 时必须用 `"..."` 包裹（内部 `\"` `\\` 转义）。仅含顶点名（无操作符）的行创建孤立点。
...
| `A-3--B` | 无向边，权 3 |
| `A-->B` | 有向边 A→B，权 1.0 |
| `A->B` | 有向边 A→B（最简），权 1.0 |
| `A<-B` | 有向边 B→A（最简），权 1.0 |
...
**关节点 & 桥 (Tarjan)**
- 单次 DFS，预建无向化邻接表确保有向边双向可见
- 桥判定 `low[v] > disc[u]`，平行边自动去重（须全部断开才判定为桥）
- 根节点 `children > 1` 为割点
...
- **分量独立布局**：多连通分量图先拆分为独立分量，每个分量在自己的网格单元内运行 FR，互不干扰，彻底解决多分量散落边缘问题
- **初始分布**：环形均匀放置（半径 = 35% 短边），每顶点加 ±10px 随机抖动
- **排斥力**：所有顶点对之间，`1.0 × k² / dist`（标准 FR 公式）
- **吸引力**：每对邻接顶点仅一次（平行边去重），`dist² / k`
- **理想边长**：`k = 0.8 × √(area / n)`，紧凑度系数 0.8 确保布局在可用空间内自然收束
- **降温**：乘法降温 200 次迭代，末温 ≈ 初温 1%
- **软边界**：边距 8% 画布短边，越界时 80% 温和拉回，避免硬钳制导致顶点共线
- **小图增强**：≤4 顶点的分量理想间距放大 1.2 倍
...
手动字符扫描（非纯正则），6 阶段结构化解析（v1.2.2 严格化）：

1. **预处理**：`#` 截断注释，去除首尾空白
2. **顶点解析**：`"..."` 引号模式支持转义 `\"` 和 `\\`；无引号名遇 `-` `<` `>` 即停
3. **孤立点检测**：若左顶点后无操作符（行尾/仅剩空白），创建孤立点
4. **操作符解析**（x-W-y 结构）：
   - 收集 `[-\d.<>]` + 引号子串
   - 末尾数字裁剪（防止贪吃右顶点）
   - 识别可选权重 W（浮点数或引号字符串；非法值报错）
   - **W 缺失**：x+y 合计 ≥ 1 个 `-`
   - **W 存在**：x、y 各自 ≥ 1 个 `-`
   - `<` 仅限 x 最左、`>` 仅限 y 最右，各至多一次
5. **方向判定**：`<` → 反向有向，`>` → 正向有向，同有 → 无向，皆无 → 无向
6. **尾部校验**：右顶点解析后行尾不得有非空白内容
7. **错误上报**：行号 + 具体原因，不中断其余行解析
